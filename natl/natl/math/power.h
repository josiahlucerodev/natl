...
namespace natl::math {
	...
	namespace impl {
		NATL_FORCE_INLINE constexpr f32 basicSqrtF32ImplS(const f32 value) noexcept {
			...
			const f32 x = bitCast<f32, i32>(static_cast<i32>(sqrtMagicNumber - static_cast<f32>(bitCast<ui32, f32>(value) >> 1)));
		...
		NATL_FORCE_INLINE constexpr f64 basicSqrtF64ImplS(const f64 value) noexcept {
			...
			const f64 x = bitCast<f64, i64>(static_cast<i64>(sqrtMagicNumber - static_cast<f64>(bitCast<ui64, f64>(value) >> 1)));
		...
		NATL_FORCE_INLINE f64 basicSqrtF64ImplD(const f64 value) noexcept {
			return static_cast<f64>(__builtin_sqrtl(static_cast<long double>(value)));
		}
...
#else 
		static_assert(false, "natl: no implemenation of dependent basic sqrt");
#endif 
	}
	...
	namespace impl {
		...
		NATL_FORCE_INLINE f32 sqrtF32ImplD(const f32 value) noexcept {
			return basicSqrtF32ImplD(value);
		}
		NATL_FORCE_INLINE f64 sqrtF64ImplD(const f64 value) noexcept {
			return basicSqrtF64ImplD(value);
		}
#elif defined(NATL_COMPILER_GCC) && defined(NATL_ARCHITECTURE_X86_64)
		NATL_FORCE_INLINE f32 sqrtF32ImplD(const f32 value) noexcept {
			return __builtin_sqrtf(value);
...
#else 
		static_assert(false, "natl: no implemenation of dependent sqrt");
#endif 
	}
	...
	namespace impl {
		...
		NATL_FORCE_INLINE f32 basicRsqrtF32ImplD(const f32 value) noexcept {
			using namespace natl::literals;
			return 1.0_f32 / basicSqrtF32ImplD(value);
		}
		NATL_FORCE_INLINE f64 basicRsqrtF64ImplD(const f64 value) noexcept {
			using namespace natl::literals;
			return 1.0_f64 /  basicSqrtF64ImplD(value);
		}
#elif defined(NATL_COMPILER_GCC) && defined(NATL_ARCHITECTURE_X86_64)
		NATL_FORCE_INLINE f32 basicRsqrtF32ImplD(const f32 value) noexcept {
			using namespace natl::literals;
			return 1.0_f32 / __builtin_sqrtf(value);
		}
		NATL_FORCE_INLINE f64 basicRsqrtF64ImplD(const f64 value) noexcept {
			using namespace natl::literals;
			return 1.0_f64 / static_cast<f64>(__builtin_sqrtl(static_cast<long double>(value)));
		}
#elif defined(NATL_COMPILER_MSVC) && defined(NATL_ARCHITECTURE_X86_64)
		NATL_FORCE_INLINE f32 basicRsqrtF32ImplD(const f32 value) noexcept {
...
#else 
		static_assert(false, "natl: no implemenation of dependent basic rsqrt");
#endif 
	}
...
#else 
		static_assert(false, "natl: no implemenation of dependent rsqrt");
#endif 
	}
	...
	NATL_FORCE_INLINE constexpr f64 basicHypotF64(const f64 x, const f64 y, const f64 z) noexcept {
	...
	NATL_FORCE_INLINE constexpr f64 hypotF64(const f64 x, const f64 y, const f64 z) noexcept {
